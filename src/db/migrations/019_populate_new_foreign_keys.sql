...
begin
  if exists (
    ...
    where table_name = 'fights' and column_name = 'fighter_a_id'
  ) then
    update fights f
...
begin
  if exists (
    select 1 from information_schema.columns 
    where table_name = 'fights' and column_name = 'fighter_b_id'
  ) then
    update fights f
...
begin
  if exists (
    select 1 from information_schema.columns 
    where table_name = 'events' and column_name = 'plo_id'
  ) then
    update events e
...
begin
  if exists (
    select 1 from information_schema.columns 
    where table_name = 'offers' and column_name = 'fighter_id'
  ) then
    update offers o
...
begin
  if exists (
    select 1 from information_schema.columns 
    where table_name = 'offers' and column_name = 'plo_id'
  ) then
    update offers o
    set plo_profile_id = pp.id
    from plo_profiles pp
...
begin
  if exists (
    select 1 from information_schema.columns 
    where table_name = 'fighter_verifications' and column_name = 'fighter_id'
  ) then
