...
export async function all(): Promise<Offer[]> {
  const r = await query('select id, fight_id as "fightId", event_id as "eventId", event_slot_id as "eventSlotId", fighter_id as "fighterId", plo_id as "ploId", amount, currency, created_at as "createdAt" from offers order by created_at desc');
...
export async function create(fightId: string, eventId: string, eventSlotId: string, fighterId: string, ploId: string, amount: number, currency: string): Promise<Offer> {
  const r = await query('insert into offers (fight_id, event_id, event_slot_id, fighter_id, plo_id, amount, currency) values ($1, $2, $3, $4, $5, $6, $7) returning id, fight_id as "fightId", event_id as "eventId", event_slot_id as "eventSlotId", fighter_id as "fighterId", plo_id as "ploId", amount, currency, created_at as "createdAt"', [fightId, eventId, eventSlotId, fighterId, ploId, amount, currency]);
  return r.rows[0];
}
...
export async function findExistingOffer(fightId: string, ploId: string): Promise<Offer | null> {
  const r = await query('select id, fight_id as "fightId", event_id as "eventId", event_slot_id as "eventSlotId", fighter_id as "fighterId", plo_id as "ploId", amount, currency, created_at as "createdAt" from offers where fight_id=$1 and plo_id=$2 limit 1', [fightId, ploId]);
  return r.rows[0] || null;
}

export async function getById(id: string): Promise<Offer | null> {
  const r = await query('select id, fight_id as "fightId", event_id as "eventId", event_slot_id as "eventSlotId", fighter_id as "fighterId", plo_id as "ploId", amount, currency, created_at as "createdAt" from offers where id=$1', [id]);
  return r.rows[0] || null;
}
