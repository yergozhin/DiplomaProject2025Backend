...
export async function getByFightId(fightId: string): Promise<FightStatistic[]> {
  ...
  for (const row of res.rows) {
    stats.push({ ...row, fighterName: row.fighter_name });
  }
  return stats;
}
...
export async function update(id: string, fields: UpdateStatisticFields): Promise<FightStatistic | null> {
  const fieldMappings: { dbField: string, value: unknown }[] = [];
  ...
  for (const [i, mapping] of fieldMappings.entries()) {
    setExpressions.push(`${mapping.dbField} = $${i + 1}`);
  }
  const paramValues: unknown[] = [];
  for (const mapping of fieldMappings) {
    paramValues.push(mapping.value);
  }
  paramValues.push(id);
